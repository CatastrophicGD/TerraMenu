        ...
        if ((IsInGame() || IsInLobby())) {
            ...
            if (!State.PanicMode && State.ModDetection && __this == *Game::pLocalPlayer && (IsInLobby() || State.BroadcastedMod == 1)) {
                ...
                if (State.rpcCooldown <= 0) {
                    //TerraMenu users can detect this rpc
            ...
            if (State.ShowPlayerInfo && IsInLobby() && !State.PanicMode)
            {
                ...
                std::string localPlayerMod = State.modUsers.find((*Game::pLocalPlayer)->fields.PlayerId) == State.modUsers.end() ? "<#ff006c>TerraMenu</color>" : State.modUsers.at((*Game::pLocalPlayer)->fields.PlayerId);
...
void dNetworkedPlayerInfo_Deserialize(NetworkedPlayerInfo* __this, MessageReader* reader, bool initialState, MethodInfo* method) {
    ...
    if (std::find(State.BlacklistFriendCodes.begin(), State.BlacklistFriendCodes.end(), friendCode) != State.BlacklistFriendCodes.end()) {
        if (State.Enable_SMAC) {
            ...
            case 2:
            {
                String* newName = convert_to_string(name + " has been kicked by <#ff006c>TerraMenu</color> <#9ef>Anticheat</color>! Reason: Blacklisted<size=0>");
                if (IsHost()) {
            ...
            case 3:
            {
                String* newName = convert_to_string(name + " has been banned by <#ff006c>TerraMenu</color> <#9ef>Anticheat</color>! Reason: Blacklisted<size=0>");
                if (IsHost()) {
                    PlayerControl_CmdCheckName(GetPlayerControlById(id), newName, NULL);
