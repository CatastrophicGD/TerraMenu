...
void updateCharCounterText(FreeChatInputField* freeChatField) {
	...
	bool commandsAllowed = State.ReadAndSendTerraChat || State.ExtraCommands;
	...
	if (!State.PanicMode && (IsInGame() || IsInLobby())) {
		for (auto playerData : GetAllPlayerData()) {
			...
			if (playerName == NetworkedPlayerInfo_get_PlayerName(playerData, nullptr)) {
				...
				if (State.IsProcessingTerraChat) {
					std::string prefix = "<size=90%><#fb0>[<#ff006c>TerraChat</color>]</color> ";
...
void dChatController_SendFreeChat(ChatController* __this, MethodInfo* method) {
	...
	if (!State.PanicMode) {
		...
		if (State.ReadAndSendTerraChat && chatTextLower.substr(0, 4) == "/sc " && chatText.substr(4) != "") {
			if (IsInGame()) State.rpcQueue.push(new RpcForceTerraChat(PlayerSelection(playerToChatAs), chatText.substr(4), true));
			if (IsInLobby()) State.lobbyRpcQueue.push(new RpcForceTerraChat(PlayerSelection(playerToChatAs), chatText.substr(4), true));
