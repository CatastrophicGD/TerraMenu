...
namespace AboutTab {
    ...
    void Render() {
        ...
        if (openWelcome) {
            ...
            ImGui::SameLine(0.0f, 0.0f);
            ImGui::TextColored(GoldCol, std::format(" {}", State.TerraVersion).c_str());
            ImGui::SameLine(0.0f, 0.0f);
            ImGui::Text(" by ");
            ImGui::SameLine(0.0f, 0.0f);
            ...
            if (State.TerraVersion.find("pr") != std::string::npos || State.TerraVersion.find("rc") != std::string::npos) {
                ...
                BoldText("Make sure you have downloaded the latest version of TerraMenu from GitHub or our", ImVec4(0.f, 1.f, 0.f, 1.f));
