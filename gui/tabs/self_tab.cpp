...
namespace SelfTab {
    ...
    void Render() {
        ...
        if (openVisuals) {
            ...
            ImGui::SameLine();
            if (ToggleButton("Read and Send TerraChat", &State.ReadAndSendTerraChat)) {
                State.Save();
            }
            if (State.ReadAndSendTerraChat) ImGui::Text("Send TerraChat messages in regular chat by typing \"/sc [message]\"!");
