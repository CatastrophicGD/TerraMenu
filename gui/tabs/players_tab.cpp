        ...
        if ((IsInGame() || IsInLobby())) {
                    ...
                    if (outfit) {
                        ...
                        if (IsInMultiplayerGame() || IsInLobby()) {
                            ...
                            if (isNameLocked && isBlacklisted) {
                                ...
                                else if (State.modUsers.at(pid) == "<#ff006c>TerraMenu</color>")
            ...
            if (selectedPlayer.has_value() && !selectedPlayer.is_Disconnected() && selectedPlayers.size() == 1)
            {
                ...
                if (it != g_PlayerCache.end())
                {
                    ...
                    if (!selectedPlayer.get_PlayerControl()->fields.notRealPlayer && selectedPlayer.get_PlayerData() != NULL) {
                        ...
                        if (isUsingMod) ImGui::Text("Client Name: %s", selectedPlayer.is_LocalPlayer() ? "TerraMenu" : RemoveHtmlTags(State.modUsers.at(selectedPid)).c_str());
            ...
            if (openTrolling && selectedPlayer.has_value()) {
                ...
                if (State.selectedPlayers.size() == 1) {
                    ...
                    if (AnimatedButton(!State.SafeMode ? "Force TerraChat" : "Fake TerraChat")) {
                        if (IsInGame()) State.rpcQueue.push(new RpcForceTerraChat(selectedPlayer, scMessage, !State.SafeMode));
                        if (IsInLobby()) State.lobbyRpcQueue.push(new RpcForceTerraChat(selectedPlayer, scMessage, !State.SafeMode));
                    }
