...
void FeatureTag(FeatureRequirement req) {
    ...
    const char* tip;
    switch (req) {
        case FeatureRequirement::Anyone:
            col   = ImVec4(0.20f, 0.78f, 0.25f, 1.f);
            label = "[Free]";
            tip   = "Free — anyone can use this, no host or safe mode required";
            break;
        case FeatureRequirement::HostOnly:
            col   = ImVec4(1.00f, 0.78f, 0.10f, 1.f);
            label = "[Host Only]";
            tip   = "Host Only — you must be the lobby host to use this feature";
            break;
        case FeatureRequirement::SafeModeOff:
        default:
            col   = ImVec4(1.00f, 0.35f, 0.20f, 1.f);
            label = "[Safe Off]";
            tip   = "Safe Mode Off — disable Safe Mode in Settings to use this feature";
            break;
    }
    SameLine(0, 3);
    TextColored(col, label);
    if (IsItemHovered()) SetTooltip("%s", tip);
}
