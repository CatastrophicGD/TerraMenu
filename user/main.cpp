...
bool GameVersionCheck() {
    ...
    /*if (!IsWindows10OrGreater()) {
        ...
        MessageBox(NULL, L"This version of Windows is not supported!", L"TerraMenu", MB_OK | MB_ICONERROR | MB_SYSTEMMODAL);
        return false;
    }*/

    if (!std::filesystem::exists(gameAssembly)) {
        Log.Error("GameAssembly.dll was not found");
        MessageBox(NULL, L"Unable to locate GameAssembly.dll", L"TerraMenu", MB_OK | MB_ICONERROR | MB_SYSTEMMODAL);
        return false;
    }
...
#define GAME_STATIC_POINTER(f,c,m) \
    ...
    { \
        if (!cctor_finished(c##__TypeInfo->_0.klass)) { \
            if (MessageBox(NULL, \
                L"TerraMenu does not support Among Us versions past v16.0.5 as of now!\n\nMake sure you downgrade your Among Us instance to v16.0.0 / v16.0.2 and use TerraMenu. If you're wondering about playing with players on the latest version, your game's version will automatically be spoofed so you can play with them!\n\nClick OK to exit the game. Your browser will then open a downgrading guide.", \
                L"TerraMenu", MB_ICONINFORMATION)) { \
...
void Run(LPVOID lpParam) {
    ...
    {
        std::ostringstream ss;
        ss << "\n\tTerraMenu - " << __DATE__ << " - " << __TIME__ << std::endl; // Log TerraMenu info
        /*ss << "\tBuild: " << _CONFIGURATION_NAME << std::endl;
        ss << "\tCommit: " << GetGitCommit() << " - " << GetGitBranch() << std::endl; // Log git info*/
        ss << "\tVersion: " << State.TerraVersion << std::endl;
        ss << "\tAmong Us Version: " << getGameVersion() << std::endl; // Log among us info
        LOG_INFO(ss.str());
#if _DEBUG
        SetConsoleTitleA(std::format("Debug Console - TerraMenu {} (Among Us v{})", State.TerraVersion, getGameVersion()).c_str());
#endif
    }
#if _DEBUG
