...
void ShowHudNotification(std::string text) {
    return;
    std::string notificationText = "</size><#fb0>[<#ff006c>TerraMenu</color>]</color> " + text + "<size=0>";
    if (IsInGame() || IsInLobby())
...
std::vector<std::string> GetAllConfigs() {
    ...
    auto configsPath = path.parent_path() / "terra-config";
...
bool CheckConfigExists(std::string configName) {
    auto path = getModulePath(hModule);
    auto settingsPath = path.parent_path() / std::format("terra-config/{}.json", State.selectedConfig);
...
void SMAC_OnCheatDetected(PlayerControl* pCtrl, std::string reason) {
    ...
    if (IsHost()) {
        ...
        case 0:
            LOG_INFO((name + " has been detected by TerraMenu Anticheat! Reason: " + reason).c_str());
            break;
        case 1: {
        ...
        case 2:
        {
            String* newName = convert_to_string(name + " <#fff>has been kicked by <#ff006c>TerraMenu</color> <#9ef>Anticheat</color>! Reason: </color><#f00><b>" + reason + "</b></color><size=0>");
            if (name.find(" by TerraMenu Anticheat! Reason: ") == std::string::npos)
        ...
        case 3:
        {
            String* newName = convert_to_string(name + " <#fff>has been banned by <#ff006c>TerraMenu</color> <#9ef>Anticheat</color>! Reason: </color><#f00><b>" + reason + "</b></color><size=0>");
            if (name.find(" by TerraMenu Anticheat! Reason: ") == std::string::npos)
                GetPlayerOutfit(GetPlayerData(pCtrl))->fields.PlayerName = newName; // Set name for yourself as it doesn't show up fast enough for others
