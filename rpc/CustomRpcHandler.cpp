...
void HandleRpc(PlayerControl* player, uint8_t callId, MessageReader* reader) {
	...
	case (uint8_t)420:
	{
		...
		if (State.modUsers.find(playerId) == State.modUsers.end() && MessageReader_get_BytesRemaining(reader, NULL) == 0) {
			State.modUsers.insert({ playerId, "<#ff006c>TerraMenu</color>" });
			STREAM_DEBUG("RPC Received for another TerraMenu user from " << ToString((Game::PlayerId)playerId));
			if (State.SMAC_CheckTerra) SMAC_OnCheatDetected(player, "TerraMenu User");
		}
	}
	break;
	case (uint8_t)42069:
