...
void HandleRpc(PlayerControl* player, uint8_t callId, MessageReader* reader) {
        ...
        case (uint8_t)420:
        {
                uint8_t playerId = player->fields.PlayerId; //true TerraMenu detection
                if (State.modUsers.find(playerId) == State.modUsers.end() && MessageReader_get_BytesRemaining(reader, NULL) == 0) {
                        State.modUsers.insert({ playerId, "<#ff6c00>SickoMenu/MalumMenu/HyperMenu</color>" });
                        STREAM_DEBUG("RPC Received for a SickoMenu/MalumMenu/HyperMenu user from " << ToString((Game::PlayerId)playerId));
                        if (State.SMAC_CheckTerra) SMAC_OnCheatDetected(player, "SickoMenu/MalumMenu/HyperMenu User");
                }
        }
        break;
        case (uint8_t)42069:
        ...
        case (uint8_t)101:
        {
                ...
                if (++State.rpc101Counter > State.RPC101_LIMIT) {
                        if (player && player != *Game::pLocalPlayer) {
                                ...
                                if (it == State.Rpc101OverloadTimestamps.end() || duration_cast<seconds>(now - it->second).count() >= 3) {
                                        ...
                                        std::string actionMessage = std::format("<b>Player <#FFF>\"{}\"</color> has exceeded the rate-limit of TerraChat!</b>", name);
                ...
                if (!State.PanicMode && State.ReadAndSendTerraChat) {
                        ...
                        State.IsProcessingTerraChat = true;
                        ChatController_AddChat(Game::HudManager.GetInstance()->fields.Chat, player, convert_to_string(message), false, NULL);
                        State.IsProcessingTerraChat = false;

                        if (wasDead) {
                                local->fields.IsDead = false;
                        }

                        STREAM_DEBUG("TerraChat RPC from " << playerName << " (RPC sent by " << ToString((Game::PlayerId)player->fields.PlayerId) << ")");
                }
        }
        break;
        }
