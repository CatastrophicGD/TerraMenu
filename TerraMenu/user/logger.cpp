...
void TerraLogger::Create()
{
	const auto path = getModulePath(NULL);
	const auto logPath = path.parent_path() / "terra-log.txt";
	const auto prevLogPath = path.parent_path() / "terra-prev-log.txt";
...
void TerraLogger::Write(std::string_view verbosity, std::string_view source, std::string_view message, bool write)
{
	std::stringstream ss;
...
void TerraLogger::Debug(std::string_view source, std::string_view message, bool write)
{
	Write("DEBUG", source, message, write);
}

void TerraLogger::Error(std::string_view source, std::string_view message)
{
	Write("ERROR", source, message);
}

void TerraLogger::Info(std::string_view source, std::string_view message)
{
	Write("INFO", source, message);
}

void TerraLogger::Debug(std::string_view message)
{
	Debug("SICKO", message);
}

void TerraLogger::Error(std::string_view message)
{
	Error("SICKO", message);
}

void TerraLogger::Info(std::string_view message)
{
	Info("SICKO", message);
}