...
#define JSON_TRYGET(key, value) \
        ...
        //Log.Info("Unable to load terra-selected-config.json");
    }

    auto settingsPath = path.parent_path() / std::format("terra-config/{}.json", this->selectedConfig);
...
#define JSON_TRYGET(key, value) \
        ...
        JSON_TRYGET("ReadAndSendTerraChat", this->ReadAndSendTerraChat);
        ...
        JSON_TRYGET("SMAC_CheckTerra", this->SMAC_CheckTerra);
        ...
        Log.Info("Unable to load " + std::format("terra-config/{}.json", this->selectedConfig));
    }
...
void Settings::SaveConfig() {
    auto path = getModulePath(hModule);
    std::filesystem::create_directory(path.parent_path() / "terra-config");
    ...
    if (this->selectedConfig != "") {
        try {
            ...
            //Log.Info("Unable to save terra-selected-config.json");
        }
    }
}
...
void Settings::Save() {
    ...
    if (this->selectedConfig != "") {
        ...
        auto settingsPath = path.parent_path() /
            std::format("terra-config/{}.json", GetAllConfigs().size() != 0 ? this->selectedConfig : "default");
        ...
        try {
            nlohmann::ordered_json j = nlohmann::ordered_json{
                ...
                { "ReadAndSendTerraChat", this->ReadAndSendTerraChat },
                ...
                { "SMAC_CheckTerra", this->SMAC_CheckTerra },
            ...
            Log.Info("Unable to save " + std::format("terra-config/{}.json", this->selectedConfig));
        }
...
void Settings::Delete() {
    ...
    auto configPath = path.parent_path() / std::format("terra-config/{}.json", this->selectedConfig);
